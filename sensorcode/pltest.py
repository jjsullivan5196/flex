...
for series,color in zip(smoothdata,colors):
    smoothlines.extend(smooth.plot(series, color, linewidth = 10))

for series,color in zip(slopedata,colors):
    slopelines.extend(slope.plot(series, color, linewidth = 10))
...
ani = animation.FuncAnimation(fig, animate, sensor.sample(), init_func=init, interval=0, blit = True)
